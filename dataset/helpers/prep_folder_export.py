...
def prepare_folder_export(
    ...
    try:
        # check in exports already done
        verify_export_integrity(folder["folder"] / "export")
        return
    except Exception as e:
        print("Previous exports not made")
    ...
    if len(processed_script) != len(audio_chunks):
        ...
        print(
            f"Sentences: {len(processed_script)=}, Audio chunks: {len(audio_chunks)=}"
        )
        return
    ...
    try:
        verify_export_integrity(export_folder)
    except Exception as e:
        print(f"Failed to verify export prep operation on folder: {folder['folder']}")
        print(str(e))
